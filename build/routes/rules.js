...
function parseRuleBody(body) {
    const phone = (0, activeRules_1.normalizePhone)(body?.phone);
...
function createRulesRouter(store) {
    ...
    r.patch("/:phone", async (req, res) => {
        try {
            const phone = (0, activeRules_1.normalizePhone)(req.params.phone);
            if (!phone)
                throw new Error("Invalid phone param");
