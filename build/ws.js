...
function createWsServer(server, handlers = {}) {
    const { onBroadcast, onLogout } = handlers;
    const wss = new ws_1.WebSocketServer({ server, path: config_1.WS_PATH });
    ...
    wss.on("connection", (ws) => {
        ...
        ws.on("message", async (raw) => {
            ...
            try {
                if (action === "clear_all") {
                    ...
                    try {
                        ws.send(JSON.stringify({ type: "cmd_ack", data: { action: "clear_all" } }));
                    }
                    catch { }
                ...
                if (action === "clear_client") {
                    ...
                    if (!clientId)
                        return;
                    store_1.store.clearClient(clientId);
                    try {
                        ws.send(JSON.stringify({ type: "cmd_ack", data: { action: "clear_client", clientId } }));
                    }
                    catch { }
                ...
                if (action === "broadcast") {
                    if (broadcastInFlight) {
                        try {
                            ws.send(JSON.stringify({ type: "cmd_ack", data: { action: "broadcast", ok: true, skipped: true } }));
                        }
                        catch { }
                        return;
                    }
                    ...
                    try {
                        ws.send(JSON.stringify({ type: "cmd_ack", data: { action: "broadcast", ok: true } }));
                    }
                    catch { }
                    try {
                        await onBroadcast?.();
                ...
                if (action === "logout") {
                    try {
                        ws.send(JSON.stringify({ type: "cmd_ack", data: { action: "logout", ok: true } }));
                    }
                    catch { }
                    await onLogout?.();
                    return;
        ...
        try {
            ws.send(JSON.stringify({ type: "messages", data: store_1.store.snapshot() }));
        }
        catch { }
        try {
            ws.send(JSON.stringify({ type: "status", data: store_1.store.state }));
        }
        catch { }
    });
